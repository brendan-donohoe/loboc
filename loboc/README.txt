...
necessary, as defined in the Spike 3 specification.
...
in the Spike 4 specification) encountered to standard error.
...
R, R' <- (a "result", either a variable, a numeric literal, the unary &
addressing operator followed by a variable, or a variable followed by the unary
...
Finally, the machine code is a straightforward translation of the
three address code.
...
Optimization occurs in the scope of individual basic blocks, and during a single
round of optimization, the optimizer makes a full pass through each block,
attempting to apply each of the above optimizations as it goes.  Once the pass
is complete, as long as at least one optimization was successfully applied, it
will attempt another pass through the code until no further changes are made.

With all of the above optimizations in tow, this is the assembly code generated
with optimizations disabled for is5_signed_arith_fold6.loboc (available in the
test directory).
...
This is the code generated after optimizations have been applied.
...
Obviously, this is a little bit of an exceptional case, due to the fact that
said program has no conditional jumps or pointers, and all variables are
assigned to constants in the code itself.  But, it still serves to illustrate
the power of the optimizations in sections of code where they do end up coming
in handy.
...
Type Error: Occurs if operands of an operator have invalid types (according to
the type propagation rules in Spike 4).  For instance:
